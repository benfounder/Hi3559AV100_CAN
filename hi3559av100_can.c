 ...
 *     2024 by Kai <jiakai1000@gmail.com>
 *
...
static int hisi_can_preread(struct hisi_can_priv *priv, u8 index)
{
    ...
    if (0 != (map & std_recv_map)) {
        ...
        /* Step 2 */
        writel(0xC000, priv->reg_base+IF2_MASK2);
    } else if (0 != (map & ext_recv_map)) {
        writel(0xC000 | (priv->ext_ids[index-1] >> 16), priv->reg_base+IF2_ARBITRATION2);
        writel(priv->ext_ids[index-1] & 0xffff, priv->reg_base+IF2_ARBITRATION1);
        map <<= 1;
        ...
        /* Step 2 */
        writel(0xDfff, priv->reg_base+IF2_MASK2);
    }
#if 0
    else if (index <= 15) {
...
#ifdef DEBUG
        printk(KERN_ERR "%s:%d index %d: end\n", __FUNCTION__, __LINE__, index);
#endif
    } else {
...
static irqreturn_t hisi_can_irq(int irq, void *dev_id)
{
    ...
    if (reg_interrupt == 0x8000) {
...
#ifdef DEBUG
        printk(KERN_ERR "%s:%d reg pending 0x%x\n", __FUNCTION__, __LINE__, reg_pending);
        ...
        reg_tmp = reg_pending & (std_recv_map | ext_recv_map);
#ifdef DEBUG
...
static int hisi_can_open(struct net_device *dev)
{
    ...
    for (index = 0; index < 32; index ++) {
        if (std_recv_map & (1UL << index)) {
            netdev_info(dev, "Listen standard data packet on mailbox %u\n", index+1);
            hisi_can_preread(priv, index+1);
        } else if (ext_recv_map & (1UL << index)) {
            netdev_info(dev, "Listen extend data packet on mailbox %u, ID 0x%x\n", index+1, priv->ext_ids[index]);
            hisi_can_preread(priv, index+1);
        }
    }
...
static int hisi_can_probe(struct platform_device *pdev)
{
    ...
    // TODO set to your own IDs.
    priv->ext_ids[12] = 0x5678;
    priv->ext_ids[13] = 0x800;
    priv->ext_ids[14] = 0x789a;
    priv->ext_ids[15] = 0x900;
